// Copyright (c) Robin Mordasiewicz
// SPDX-License-Identifier: MPL-2.0

package rate_limiter_policy

import (
	"github.com/hashicorp/terraform-plugin-framework/types"
)

// RateLimiterPolicyResourceModel describes the resource data model.
type RateLimiterPolicyResourceModel struct {
	ID          types.String `tfsdk:"id"`
	Name        types.String `tfsdk:"name"`
	Namespace   types.String `tfsdk:"namespace"`
	Description types.String `tfsdk:"description"`
}

// APIRateLimiterPolicy represents the API request/response structure.
type APIRateLimiterPolicy struct {
	Metadata   APIMetadata              `json:"metadata"`
	Spec       APIRateLimiterPolicySpec `json:"spec"`
	SystemMeta *APISystemMetadata       `json:"system_metadata,omitempty"`
}

// APIMetadata represents the metadata section.
type APIMetadata struct {
	Name        string `json:"name"`
	Namespace   string `json:"namespace,omitempty"`
	Description string `json:"description,omitempty"`
}

// APISystemMetadata represents system-generated metadata.
type APISystemMetadata struct {
	UID string `json:"uid,omitempty"`
}

// APIRateLimiterPolicySpec represents the Rate Limiter Policy specification.
type APIRateLimiterPolicySpec struct {
}

// ToAPIRequest converts the Terraform model to API request format.
func (m *RateLimiterPolicyResourceModel) ToAPIRequest() *APIRateLimiterPolicy {
	apiReq := &APIRateLimiterPolicy{
		Metadata: APIMetadata{
			Name:        m.Name.ValueString(),
			Namespace:   m.Namespace.ValueString(),
			Description: m.Description.ValueString(),
		},
		Spec: APIRateLimiterPolicySpec{},
	}

	return apiReq
}

// FromAPIResponse updates the Terraform model from API response.
func (m *RateLimiterPolicyResourceModel) FromAPIResponse(resp *APIRateLimiterPolicy) {
	m.Name = types.StringValue(resp.Metadata.Name)
	m.Namespace = types.StringValue(resp.Metadata.Namespace)

	if resp.Metadata.Description != "" {
		m.Description = types.StringValue(resp.Metadata.Description)
	}

	if resp.SystemMeta != nil && resp.SystemMeta.UID != "" {
		m.ID = types.StringValue(resp.SystemMeta.UID)
	} else {
		m.ID = types.StringValue(resp.Metadata.Namespace + "/" + resp.Metadata.Name)
	}
}
