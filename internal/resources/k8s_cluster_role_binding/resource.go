...
var _ resource.Resource = &K8sClusterRoleBindingResource{}
var _ resource.ResourceWithImportState = &K8sClusterRoleBindingResource{}

type K8sClusterRoleBindingResource struct {
	client *client.Client
}

func NewK8sClusterRoleBindingResource() resource.Resource { return &K8sClusterRoleBindingResource{} }
...
func (r *K8sClusterRoleBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	tflog.Debug(ctx, "Creating K8s Cluster Role Binding", map[string]interface{}{"name": data.Name.ValueString()})
	apiReq := data.ToAPIRequest()
	var apiResp APIK8sClusterRoleBinding
	path := fmt.Sprintf("/config/namespaces/%s/k8s_cluster_role_bindings", data.Namespace.ValueString())
	if err := r.client.Post(ctx, path, apiReq, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error creating K8s Cluster Role Binding", err.Error())
		return
	}
...
func (r *K8sClusterRoleBindingResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	var apiResp APIK8sClusterRoleBinding
	path := fmt.Sprintf("/config/namespaces/%s/k8s_cluster_role_bindings/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := r.client.Get(ctx, path, &apiResp); err != nil {
		if client.IsNotFound(err) {
			resp.State.RemoveResource(ctx)
			return
		}
		resp.Diagnostics.AddError("Error reading K8s Cluster Role Binding", err.Error())
		return
	}
	data.FromAPIResponse(&apiResp)
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
...
func (r *K8sClusterRoleBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	tflog.Debug(ctx, "Updating K8s Cluster Role Binding", map[string]interface{}{"name": data.Name.ValueString()})
	apiReq := data.ToAPIRequest()
	var apiResp APIK8sClusterRoleBinding
	path := fmt.Sprintf("/config/namespaces/%s/k8s_cluster_role_bindings/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := r.client.Put(ctx, path, apiReq, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error updating K8s Cluster Role Binding", err.Error())
		return
	}
	data.FromAPIResponse(&apiResp)
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
...
func (r *K8sClusterRoleBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting K8s Cluster Role Binding", map[string]interface{}{"name": data.Name.ValueString()})
	path := fmt.Sprintf("/config/namespaces/%s/k8s_cluster_role_bindings/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := r.client.Delete(ctx, path); err != nil {
		resp.Diagnostics.AddError("Error deleting K8s Cluster Role Binding", err.Error())
		return
	}
}
...
func (r *K8sClusterRoleBindingResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	var apiResp APIK8sClusterRoleBinding
	path := fmt.Sprintf("/config/namespaces/%s/k8s_cluster_role_bindings/%s", parts[0], parts[1])
	if err := r.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error importing K8s Cluster Role Binding", err.Error())
		return
	}
	data.FromAPIResponse(&apiResp)
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
