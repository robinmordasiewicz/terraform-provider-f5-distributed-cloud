...
func (r *WAFExclusionPolicyResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	tflog.Debug(ctx, "Creating WAF Exclusion Policy", map[string]interface{}{
		"name": data.Name.ValueString(), "namespace": data.Namespace.ValueString(),
	})

	apiReq := data.ToAPIRequest()
	var apiResp APIWAFExclusionPolicy
	path := fmt.Sprintf("/config/namespaces/%s/waf_exclusion_policys", data.Namespace.ValueString())
	if err := r.client.Post(ctx, path, apiReq, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Creating WAF Exclusion Policy",
			fmt.Sprintf("Could not create WAF Exclusion Policy %s/%s: %s",
...
func (r *WAFExclusionPolicyResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	var apiResp APIWAFExclusionPolicy
	path := fmt.Sprintf("/config/namespaces/%s/waf_exclusion_policys/%s",
	...
	if err := r.client.Get(ctx, path, &apiResp); err != nil {
		...
		resp.Diagnostics.AddError("Error Reading WAF Exclusion Policy",
			fmt.Sprintf("Could not read WAF Exclusion Policy %s/%s: %s",
				data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
...
func (r *WAFExclusionPolicyResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if err := r.client.Put(ctx, path, apiReq, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Updating WAF Exclusion Policy",
			fmt.Sprintf("Could not update WAF Exclusion Policy %s/%s: %s",
				data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
...
func (r *WAFExclusionPolicyResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err := r.client.Delete(ctx, path); err != nil {
		...
		resp.Diagnostics.AddError("Error Deleting WAF Exclusion Policy",
			fmt.Sprintf("Could not delete WAF Exclusion Policy %s/%s: %s",
				data.Namespace.ValueString(), data.Name.ValueString(), err))
	}
}
