...
var _ resource.Resource = &AzureVNETSiteResource{}
var _ resource.ResourceWithImportState = &AzureVNETSiteResource{}

type AzureVNETSiteResource struct {
	client *client.Client
}

func NewAzureVNETSiteResource() resource.Resource { return &AzureVNETSiteResource{} }
...
func (r *AzureVNETSiteResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	tflog.Debug(ctx, "Creating Azure VNET Site", map[string]interface{}{"name": data.Name.ValueString()})
	apiReq := data.ToAPIRequest()
	var apiResp APIAzureVNETSite
	path := fmt.Sprintf("/config/namespaces/%s/azure_vnet_sites", data.Namespace.ValueString())
	if err := r.client.Post(ctx, path, apiReq, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error creating Azure VNET Site", err.Error())
		return
	}
...
func (r *AzureVNETSiteResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err := r.client.Get(ctx, path, &apiResp); err != nil {
		...
		resp.Diagnostics.AddError("Error reading Azure VNET Site", err.Error())
		return
	}
	data.FromAPIResponse(&apiResp)
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
...
func (r *AzureVNETSiteResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if err := r.client.Put(ctx, path, apiReq, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error updating Azure VNET Site", err.Error())
		return
	}
	data.FromAPIResponse(&apiResp)
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
...
func (r *AzureVNETSiteResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err := r.client.Delete(ctx, path); err != nil && !client.IsNotFound(err) {
		resp.Diagnostics.AddError("Error deleting Azure VNET Site", err.Error())
	}
}
...
func (r *AzureVNETSiteResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	var apiResp APIAzureVNETSite
	path := fmt.Sprintf("/config/namespaces/%s/azure_vnet_sites/%s", parts[0], parts[1])
	if err := r.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error importing Azure VNET Site", err.Error())
		return
	}
	var data AzureVNETSiteResourceModel
	data.FromAPIResponse(&apiResp)
