...
func (r *SegmentResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	var apiResp APISegment
	path := fmt.Sprintf("/config/namespaces/%s/segments", data.Namespace.ValueString())
	if err := r.client.Post(ctx, path, apiReq, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Creating Segment",
			fmt.Sprintf("Could not create Segment %s/%s: %s",
...
func (r *SegmentResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	var apiResp APISegment
	path := fmt.Sprintf("/config/namespaces/%s/segments/%s",
	...
	if err := r.client.Get(ctx, path, &apiResp); err != nil {
		...
		resp.Diagnostics.AddError("Error Reading Segment",
			fmt.Sprintf("Could not read Segment %s/%s: %s",
				data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
...
func (r *SegmentResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if err := r.client.Put(ctx, path, apiReq, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Updating Segment",
			fmt.Sprintf("Could not update Segment %s/%s: %s",
				data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
...
func (r *SegmentResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err := r.client.Delete(ctx, path); err != nil {
		...
		resp.Diagnostics.AddError("Error Deleting Segment",
			fmt.Sprintf("Could not delete Segment %s/%s: %s",
				data.Namespace.ValueString(), data.Name.ValueString(), err))
	}
}
