...
var _ resource.Resource = &K8sClusterResource{}
...
func (r *K8sClusterResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	tflog.Debug(ctx, "Creating K8s Cluster", map[string]interface{}{"name": data.Name.ValueString()})
	apiReq := data.ToAPIRequest()
	var apiResp APIK8sCluster
	path := fmt.Sprintf("/config/namespaces/%s/k8s_clusters", data.Namespace.ValueString())
...
func (r *K8sClusterResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	var apiResp APIK8sCluster
	path := fmt.Sprintf("/config/namespaces/%s/k8s_clusters/%s", data.Namespace.ValueString(), data.Name.ValueString())
...
func (r *K8sClusterResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	tflog.Debug(ctx, "Updating K8s Cluster", map[string]interface{}{"name": data.Name.ValueString()})
	apiReq := data.ToAPIRequest()
	var apiResp APIK8sCluster
	path := fmt.Sprintf("/config/namespaces/%s/k8s_clusters/%s", data.Namespace.ValueString(), data.Name.ValueString())
...
func (r *K8sClusterResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting K8s Cluster", map[string]interface{}{"name": data.Name.ValueString()})
	path := fmt.Sprintf("/config/namespaces/%s/k8s_clusters/%s", data.Namespace.ValueString(), data.Name.ValueString())
...
func (r *K8sClusterResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	var apiResp APIK8sCluster
	path := fmt.Sprintf("/config/namespaces/%s/k8s_clusters/%s", parts[0], parts[1])
	if err := r.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error importing K8s Cluster", err.Error())
		return
	}
	data.FromAPIResponse(&apiResp)
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
