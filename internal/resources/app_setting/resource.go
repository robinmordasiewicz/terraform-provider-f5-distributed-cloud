...
var _ resource.Resource = &AppSettingResource{}
var _ resource.ResourceWithImportState = &AppSettingResource{}

type AppSettingResource struct {
	client *client.Client
}

func NewAppSettingResource() resource.Resource { return &AppSettingResource{} }
...
func (r *AppSettingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	tflog.Debug(ctx, "Creating App Setting", map[string]interface{}{"name": data.Name.ValueString()})
	apiReq := data.ToAPIRequest()
	var apiResp APIAppSetting
	path := fmt.Sprintf("/config/namespaces/%s/app_settings", data.Namespace.ValueString())
	if err := r.client.Post(ctx, path, apiReq, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error creating App Setting", err.Error())
		return
	}
...
func (r *AppSettingResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	var apiResp APIAppSetting
	path := fmt.Sprintf("/config/namespaces/%s/app_settings/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := r.client.Get(ctx, path, &apiResp); err != nil {
		if client.IsNotFound(err) {
			resp.State.RemoveResource(ctx)
			return
		}
		resp.Diagnostics.AddError("Error reading App Setting", err.Error())
		return
	}
	data.FromAPIResponse(&apiResp)
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
...
func (r *AppSettingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if err := r.client.Put(ctx, path, apiReq, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error updating App Setting", err.Error())
		return
	}
	data.FromAPIResponse(&apiResp)
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
...
func (r *AppSettingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err := r.client.Delete(ctx, path); err != nil && !client.IsNotFound(err) {
		resp.Diagnostics.AddError("Error deleting App Setting", err.Error())
	}
}
...
func (r *AppSettingResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	var apiResp APIAppSetting
	path := fmt.Sprintf("/config/namespaces/%s/app_settings/%s", parts[0], parts[1])
	if err := r.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error importing App Setting", err.Error())
		return
	}
	var data AppSettingResourceModel
	data.FromAPIResponse(&apiResp)
