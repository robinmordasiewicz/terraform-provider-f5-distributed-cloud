...
var _ resource.ResourceWithImportState = &NetworkPolicyResource{}

type NetworkPolicyResource struct {
	client *client.Client
}

func NewNetworkPolicyResource() resource.Resource { return &NetworkPolicyResource{} }
...
func (r *NetworkPolicyResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	tflog.Debug(ctx, "Creating Network Policy", map[string]interface{}{"name": data.Name.ValueString()})
	apiReq := data.ToAPIRequest()
	var apiResp APINetworkPolicy
	path := fmt.Sprintf("/config/namespaces/%s/network_policys", data.Namespace.ValueString())
...
func (r *NetworkPolicyResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	var apiResp APINetworkPolicy
	path := fmt.Sprintf("/config/namespaces/%s/network_policys/%s", parts[0], parts[1])
	if err := r.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error importing Network Policy", err.Error())
		return
	}
	var data NetworkPolicyResourceModel
	data.FromAPIResponse(&apiResp)
