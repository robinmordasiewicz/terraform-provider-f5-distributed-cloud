...
`,
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the app type.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the app type to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the app type is located.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the app type.",
				Computed:    true,
			},
...
func (d *AppTypeDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading App Type",
			fmt.Sprintf("Could not read app type %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
	...
	if apiResp.Spec.AppType != "" {
		data.AppType = types.StringValue(apiResp.Spec.AppType)
	}
