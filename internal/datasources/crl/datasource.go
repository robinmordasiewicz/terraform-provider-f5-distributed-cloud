...
var _ datasource.DataSource = &CRLDataSource{}
...
func (d *CRLDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud CRL.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the crl.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the crl to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the crl exists.",
				Required:    true,
			},
...
func (d *CRLDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APICRLDS
	path := fmt.Sprintf("/api/config/%s/crls/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading CRL",
			fmt.Sprintf("Could not read crl %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
