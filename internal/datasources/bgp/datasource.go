...
func (d *BGPDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud BGP.",
		Attributes: map[string]schema.Attribute{
			...
			"name": schema.StringAttribute{
				Description: "Name of the bgp to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the bgp exists.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the bgp.",
				Computed:    true,
			},
		},
	}
...
func (d *BGPDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APIBGPDS
	path := fmt.Sprintf("/api/config/%s/bgps/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading BGP",
			fmt.Sprintf("Could not read bgp %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
