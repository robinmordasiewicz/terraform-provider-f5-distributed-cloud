...
`,
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the GCP VPC site.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the GCP VPC site to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the GCP VPC site is located.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the GCP VPC site.",
				Computed:    true,
			},
			...
			"gcp_region": schema.StringAttribute{
				Description: "GCP region where the site is deployed.",
				Computed:    true,
			},
		},
	}
...
func (d *GCPVPCSiteDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading GCP VPC Site",
			fmt.Sprintf("Could not read GCP VPC site %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
	...
	if apiResp.Spec.GCPRegion != "" {
		data.GCPRegion = types.StringValue(apiResp.Spec.GCPRegion)
	}
