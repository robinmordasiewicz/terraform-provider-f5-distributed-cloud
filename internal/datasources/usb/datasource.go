...
var _ datasource.DataSource = &UsbDataSource{}
...
func (d *UsbDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud Usb.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the usb.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the usb to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the usb exists.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the usb.",
				Computed:    true,
			},
		},
	}
...
func (d *UsbDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *UsbDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APIUsbDS
	path := fmt.Sprintf("/api/operate/%s/usbs/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading Usb",
			fmt.Sprintf("Could not read usb %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
