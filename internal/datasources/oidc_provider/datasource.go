...
func (d *OIDCProviderDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud OIDCProvider.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the oidc_provider.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the oidc_provider to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the oidc_provider exists.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the oidc_provider.",
				Computed:    true,
			},
		},
	}
...
func (d *OIDCProviderDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APIOIDCProviderDS
	path := fmt.Sprintf("/api/web/%s/oidc_providers/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading OIDCProvider",
			fmt.Sprintf("Could not read oidc_provider %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
