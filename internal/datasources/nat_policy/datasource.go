...
var _ datasource.DataSource = &NATPolicyDataSource{}
...
func (d *NATPolicyDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud NATPolicy.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the nat_policy.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the nat_policy to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the nat_policy exists.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the nat_policy.",
				Computed:    true,
			},
		},
	}
...
func (d *NATPolicyDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APINATPolicyDS
	path := fmt.Sprintf("/api/config/%s/nat_policies/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading NATPolicy",
			fmt.Sprintf("Could not read nat_policy %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
