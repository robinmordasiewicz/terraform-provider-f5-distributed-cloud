...
type APINetworkInterfaceDS struct {
	Metadata struct {
...
func (d *NetworkInterfaceDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud NetworkInterface.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the network_interface.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the network_interface to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the network_interface exists.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the network_interface.",
				Computed:    true,
			},
		},
	}
...
func (d *NetworkInterfaceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APINetworkInterfaceDS
	path := fmt.Sprintf("/api/config/%s/network_interfaces/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading NetworkInterface",
			fmt.Sprintf("Could not read network_interface %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
