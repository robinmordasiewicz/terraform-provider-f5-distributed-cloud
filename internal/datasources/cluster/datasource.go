...
func (d *ClusterDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud Cluster.",
		Attributes: map[string]schema.Attribute{
			...
			"namespace": schema.StringAttribute{
				Description: "Namespace where the cluster exists.",
				Required:    true,
			},
...
func (d *ClusterDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APIClusterDS
	path := fmt.Sprintf("/api/config/%s/clusters/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading Cluster",
			fmt.Sprintf("Could not read cluster %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
