...
func (d *GlobalLogReceiverDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		MarkdownDescription: `
The ` + "`f5xc_global_log_receiver`" + ` data source retrieves information about an existing global log receiver.
...
`,
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the global log receiver.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the global log receiver to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the global log receiver is located.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the global log receiver.",
				Computed:    true,
			},
			"receiver_type": schema.StringAttribute{
				Description: "Type of the global log receiver.",
				Computed:    true,
			},
		},
	}
...
func (d *GlobalLogReceiverDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading Global Log Receiver",
			fmt.Sprintf("Could not read global log receiver %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
