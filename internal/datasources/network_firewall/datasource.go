...
type APINetworkFirewallDS struct {
	Metadata struct {
...
func (d *NetworkFirewallDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud NetworkFirewall.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the network_firewall.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the network_firewall to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the network_firewall exists.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the network_firewall.",
				Computed:    true,
			},
		},
	}
...
func (d *NetworkFirewallDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APINetworkFirewallDS
	path := fmt.Sprintf("/api/config/%s/network_firewalls/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading NetworkFirewall",
			fmt.Sprintf("Could not read network_firewall %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
