...
func (d *PlanDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud Plan.",
		Attributes: map[string]schema.Attribute{
			...
			"namespace": schema.StringAttribute{
				Description: "Namespace where the plan exists.",
				Required:    true,
			},
...
func (d *PlanDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APIPlanDS
	path := fmt.Sprintf("/api/web/%s/plans/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading Plan",
			fmt.Sprintf("Could not read plan %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
