	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"name": schema.StringAttribute{
				Description: "Name of the site to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the site exists.",
				Required:    true,
			},
...
func (d *SiteDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading Site",
			fmt.Sprintf("Could not read site %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
