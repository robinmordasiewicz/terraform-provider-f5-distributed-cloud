...
type APIVirtualNetworkDS struct {
	Metadata struct {
...
func (d *VirtualNetworkDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud VirtualNetwork.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the virtual_network.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the virtual_network to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the virtual_network exists.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the virtual_network.",
				Computed:    true,
			},
		},
	}
...
func (d *VirtualNetworkDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APIVirtualNetworkDS
	path := fmt.Sprintf("/api/config/%s/virtual_networks/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading VirtualNetwork",
			fmt.Sprintf("Could not read virtual_network %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
