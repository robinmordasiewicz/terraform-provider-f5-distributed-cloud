...
var _ datasource.DataSource = &AddonServiceDataSource{}
...
func (d *AddonServiceDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud AddonService.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the addon_service.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the addon_service to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the addon_service exists.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the addon_service.",
				Computed:    true,
			},
		},
	}
...
func (d *AddonServiceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APIAddonServiceDS
	path := fmt.Sprintf("/api/web/%s/addon_services/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading AddonService",
			fmt.Sprintf("Could not read addon_service %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
