...
var _ datasource.DataSource = &TrustedCAListDataSource{}
...
type APITrustedCAListDS struct {
	Metadata struct {
...
func (d *TrustedCAListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud TrustedCAList.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the trusted_ca_list.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the trusted_ca_list to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the trusted_ca_list exists.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the trusted_ca_list.",
				Computed:    true,
			},
		},
	}
...
func (d *TrustedCAListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APITrustedCAListDS
	path := fmt.Sprintf("/api/config/%s/trusted_ca_lists/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading TrustedCAList",
			fmt.Sprintf("Could not read trusted_ca_list %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
