...
func (d *FleetDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud Fleet.",
		Attributes: map[string]schema.Attribute{
			...
			"name": schema.StringAttribute{
				Description: "Name of the fleet to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the fleet exists.",
				Required:    true,
			},
...
func (d *FleetDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APIFleetDS
	path := fmt.Sprintf("/api/config/%s/fleets/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading Fleet",
			fmt.Sprintf("Could not read fleet %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
