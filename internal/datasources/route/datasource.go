...
`,
		Attributes: map[string]schema.Attribute{
			...
			"name": schema.StringAttribute{
				Description: "Name of the route to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the route is located.",
				Required:    true,
			},
			...
			"route_type": schema.StringAttribute{
				Description: "Type of the route (e.g., simple, redirect, custom).",
				Computed:    true,
			},
		},
	}
...
func (d *RouteDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading Route",
			fmt.Sprintf("Could not read route %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
	...
	if apiResp.Spec.RouteType != "" {
		data.RouteType = types.StringValue(apiResp.Spec.RouteType)
	}
