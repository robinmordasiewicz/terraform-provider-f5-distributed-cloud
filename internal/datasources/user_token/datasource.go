...
func (d *UserTokenDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud UserToken.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the user_token.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the user_token to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the user_token exists.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the user_token.",
				Computed:    true,
			},
		},
	}
...
func (d *UserTokenDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APIUserTokenDS
	path := fmt.Sprintf("/api/config/%s/user_tokens/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading UserToken",
			fmt.Sprintf("Could not read user_token %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
