...
func (d *UserGroupDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud UserGroup.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the user_group.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the user_group to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the user_group exists.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the user_group.",
				Computed:    true,
			},
		},
	}
...
func (d *UserGroupDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APIUserGroupDS
	path := fmt.Sprintf("/api/web/%s/user_groups/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading UserGroup",
			fmt.Sprintf("Could not read user_group %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
