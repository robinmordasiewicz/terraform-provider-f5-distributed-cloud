...
`,
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the Voltstack site.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the Voltstack site to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the Voltstack site is located.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the Voltstack site.",
				Computed:    true,
			},
...
func (d *VoltstackSiteDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading Voltstack Site",
			fmt.Sprintf("Could not read Voltstack site %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
