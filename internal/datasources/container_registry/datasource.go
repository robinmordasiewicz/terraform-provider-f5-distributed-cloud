...
type APIContainerRegistryDS struct {
	Metadata struct {
...
func (d *ContainerRegistryDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud ContainerRegistry.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the container_registry.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the container_registry to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the container_registry exists.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the container_registry.",
				Computed:    true,
			},
		},
	}
...
func (d *ContainerRegistryDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APIContainerRegistryDS
	path := fmt.Sprintf("/api/config/%s/container_registries/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading ContainerRegistry",
			fmt.Sprintf("Could not read container_registry %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
