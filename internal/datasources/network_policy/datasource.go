...
`,
		Attributes: map[string]schema.Attribute{
			...
			"name": schema.StringAttribute{
				Description: "Name of the network policy to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the network policy is located.",
				Required:    true,
			},
...
func (d *NetworkPolicyDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APINetworkPolicy
	path := fmt.Sprintf("/config/namespaces/%s/network_policys/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading Network Policy",
			fmt.Sprintf("Could not read network policy %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
	...
	if apiResp.Spec.PolicyType != "" {
		data.PolicyType = types.StringValue(apiResp.Spec.PolicyType)
	}
