...
func (d *DNSDomainDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud DNSDomain.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the dns_domain.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the dns_domain to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the dns_domain exists.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the dns_domain.",
				Computed:    true,
			},
		},
	}
...
func (d *DNSDomainDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APIDNSDomainDS
	path := fmt.Sprintf("/api/config/%s/dns_domains/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading DNSDomain",
			fmt.Sprintf("Could not read dns_domain %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
