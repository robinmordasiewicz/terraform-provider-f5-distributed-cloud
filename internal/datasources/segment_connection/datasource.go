...
type APISegmentConnectionDS struct {
	Metadata struct {
...
func (d *SegmentConnectionDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud SegmentConnection.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the segment_connection.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the segment_connection to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the segment_connection exists.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the segment_connection.",
				Computed:    true,
			},
		},
	}
...
func (d *SegmentConnectionDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APISegmentConnectionDS
	path := fmt.Sprintf("/api/config/%s/segment_connections/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading SegmentConnection",
			fmt.Sprintf("Could not read segment_connection %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
