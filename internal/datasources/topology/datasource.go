...
func (d *TopologyDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud Topology.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the topology.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the topology to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the topology exists.",
				Required:    true,
			},
...
func (d *TopologyDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APITopologyDS
	path := fmt.Sprintf("/api/data/%s/topologies/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading Topology",
			fmt.Sprintf("Could not read topology %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
