...
`,
		Attributes: map[string]schema.Attribute{
			...
			"namespace": schema.StringAttribute{
				Description: "Namespace where the user is located.",
				Required:    true,
			},
...
func (d *UserDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APIUser
	path := fmt.Sprintf("/config/namespaces/%s/users/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading User",
			fmt.Sprintf("Could not read user %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
	...
	if apiResp.Spec.Email != "" {
		data.Email = types.StringValue(apiResp.Spec.Email)
	}
