...
func (d *AuthenticationDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud Authentication configuration.",
		Attributes: map[string]schema.Attribute{
			...
			"name": schema.StringAttribute{
				Description: "Name of the authentication to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the authentication exists.",
				Required:    true,
			},
...
func (d *AuthenticationDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	path := fmt.Sprintf("/config/namespaces/%s/authentications/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading Authentication",
			fmt.Sprintf("Could not read authentication %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
