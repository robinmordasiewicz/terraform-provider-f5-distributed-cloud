...
func (d *SubnetDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud Subnet.",
		Attributes: map[string]schema.Attribute{
			...
			"namespace": schema.StringAttribute{
				Description: "Namespace where the subnet exists.",
				Required:    true,
			},
...
func (d *SubnetDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APISubnetDS
	path := fmt.Sprintf("/api/config/%s/subnets/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading Subnet",
			fmt.Sprintf("Could not read subnet %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
