...
func (d *WifiDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Fetches information about an existing F5 Distributed Cloud Wifi.",
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Description: "Unique identifier of the wifi.",
				Computed:    true,
			},
			"name": schema.StringAttribute{
				Description: "Name of the wifi to look up.",
				Required:    true,
			},
			"namespace": schema.StringAttribute{
				Description: "Namespace where the wifi exists.",
				Required:    true,
			},
			"description": schema.StringAttribute{
				Description: "Description of the wifi.",
				Computed:    true,
			},
		},
	}
...
func (d *WifiDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var apiResp APIWifiDS
	path := fmt.Sprintf("/api/operate/%s/wifis/%s", data.Namespace.ValueString(), data.Name.ValueString())
	if err := d.client.Get(ctx, path, &apiResp); err != nil {
		resp.Diagnostics.AddError("Error Reading Wifi",
			fmt.Sprintf("Could not read wifi %s/%s: %s", data.Namespace.ValueString(), data.Name.ValueString(), err))
		return
	}
