...
resource "f5_distributed_cloud_aws_vpc_site" "example" {
  ...
  cloud_credentials_ref = f5_distributed_cloud_cloud_credentials.aws.name
}
...
output "site_name" {
  ...
  value       = f5_distributed_cloud_aws_vpc_site.example.name
}

output "site_id" {
  description = "ID of the created AWS VPC Site"
  value       = f5_distributed_cloud_aws_vpc_site.example.id
}
