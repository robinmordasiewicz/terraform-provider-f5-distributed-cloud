...
output "tcp_lb_description" {
  ...
  value       = data.f5_distributed_cloud_tcp_loadbalancer.example.description
}
...
resource "f5_distributed_cloud_origin_pool" "backend" {
  ...
  namespace   = data.f5_distributed_cloud_tcp_loadbalancer.example.namespace
  description = "Backend pool for ${data.f5_distributed_cloud_tcp_loadbalancer.example.name}"
