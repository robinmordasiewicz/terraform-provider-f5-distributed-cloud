...
output "tcp_lb_description" {
  ...
  value       = data.f5xc_tcp_loadbalancer.example.description
}
...
resource "f5xc_origin_pool" "backend" {
  ...
  namespace   = data.f5xc_tcp_loadbalancer.example.namespace
  description = "Backend pool for ${data.f5xc_tcp_loadbalancer.example.name}"
