...
output "policy_description" {
  ...
  value       = data.f5_distributed_cloud_service_policy.example.description
}
...
resource "f5_distributed_cloud_http_loadbalancer" "protected" {
  name        = "protected-app"
  namespace   = data.f5_distributed_cloud_service_policy.example.namespace
  description = "Load balancer protected by ${data.f5_distributed_cloud_service_policy.example.name}"
  ...
  service_policy {
    name      = data.f5_distributed_cloud_service_policy.example.name
    namespace = data.f5_distributed_cloud_service_policy.example.namespace
  }
}
