...
output "rate_limiter_description" {
  ...
  value       = data.f5xc_rate_limiter.example.description
}
...
output "burst_multiplier" {
  ...
  value       = data.f5xc_rate_limiter.example.burst_multiplier
}
...
resource "f5xc_http_loadbalancer" "protected" {
  ...
  description = "Load balancer protected by ${data.f5xc_rate_limiter.example.name}"
