...
output "healthcheck_interval" {
  ...
  value       = data.f5_distributed_cloud_healthcheck.example.interval
}
...
resource "f5_distributed_cloud_origin_pool" "example" {
  ...
  namespace   = data.f5_distributed_cloud_healthcheck.example.namespace
  description = "Origin pool using ${data.f5_distributed_cloud_healthcheck.example.name} healthcheck"
  ...
  healthcheck {
    name      = data.f5_distributed_cloud_healthcheck.example.name
    namespace = data.f5_distributed_cloud_healthcheck.example.namespace
  }
}
