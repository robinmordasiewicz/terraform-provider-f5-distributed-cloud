...
output "lb_description" {
  ...
  value       = data.f5xc_http_loadbalancer.example.description
}
...
resource "f5xc_origin_pool" "backend" {
  name        = "backend-pool"
  namespace   = data.f5xc_http_loadbalancer.example.namespace
  description = "Backend pool for ${data.f5xc_http_loadbalancer.example.name}"
