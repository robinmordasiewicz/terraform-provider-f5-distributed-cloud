...
output "vsite_description" {
  ...
  value       = data.f5_distributed_cloud_virtual_site.example.description
}
...
resource "f5_distributed_cloud_http_loadbalancer" "example" {
  name        = "my-lb"
  namespace   = data.f5_distributed_cloud_virtual_site.example.namespace
  description = "Load balancer deployed to ${data.f5_distributed_cloud_virtual_site.example.name}"
  ...
  advertise_custom {
    advertise_where {
      virtual_site {
        name      = data.f5_distributed_cloud_virtual_site.example.name
        namespace = data.f5_distributed_cloud_virtual_site.example.namespace
      }
    }
  }
