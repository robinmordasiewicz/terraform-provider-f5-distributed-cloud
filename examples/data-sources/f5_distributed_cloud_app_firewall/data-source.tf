...
output "waf_id" {
  ...
  value       = data.f5_distributed_cloud_app_firewall.example.id
}

output "waf_description" {
  description = "Description of the Application Firewall"
  value       = data.f5_distributed_cloud_app_firewall.example.description
}
...
resource "f5_distributed_cloud_http_loadbalancer" "protected" {
  name        = "protected-app"
  namespace   = data.f5_distributed_cloud_app_firewall.example.namespace
  description = "Load balancer protected by ${data.f5_distributed_cloud_app_firewall.example.name}"
  ...
  app_firewall {
    name      = data.f5_distributed_cloud_app_firewall.example.name
    namespace = data.f5_distributed_cloud_app_firewall.example.namespace
  }
}
